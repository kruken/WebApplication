	...
	if (strpos($argv[0], 'doctrine-module') > 0) {
		if ($argv) {
			foreach ($argv as $key => $value) {
				if (strpos($value, '--application-env') === 0) {
					$env = str_replace('--application-env=', '', $value);
		...
		if (!$env) {
			echo "\n======================= ERROR =================================\n\n";
			echo "Missing parameter: --application-env\n\n";
			echo "Locally use: --application-env=dev-YOUR_SHORT_NAME\n\n";
			echo "On a server use: --application-env={dev,test,prod}\n";
			echo "\n===============================================================\n";
			die();
		}
